...
* Website: www.facebook/mstrhak0r
...
$mstrhak0r_error = ''; //Please leave this field alone. Do not touch it otherwise you will have problems with Error Message Displays on your system
			  

include "mstrhak0r_database.php"; //This is the file that contains the database filename for this system
$mstrhak0r_server_path = getcwd();
...
{
	if(file_exists($mstrhak0r_server_path."/mstrhak0r_database.docx"))
	{
		$mstrhak0r_array_line = array();
		array_unshift( $mstrhak0r_array_line, "<?php \n/*Defined below is your database file, the file where all your users details are saved*/\n/*if you wish to see the content of your database file then change the file mstrhak0r_database.docx to mstrhak0r_database.txt below*/\ndefine('name_of_file_to_save_user_details', 'mstrhak0r_database.docx');/*File to save and retrieve user details*/\n?>" );
		$mstrhak0r_file_ip = implode("\n",$mstrhak0r_array_line);

		$mstrhak0r_create_write_file = @fopen("mstrhak0r_database.php", "w");
		if ($mstrhak0r_create_write_file) 
		{
		  fwrite($mstrhak0r_create_write_file, $mstrhak0r_file_ip);
		  @fclose($mstrhak0r_create_write_file);
		}
	}
	else
	{
		...
		@fopen("mstrhak0r_database.docx", "w");
		$mstrhak0r_create_write_file = @fopen("mstrhak0r_database.php", "w");
		if ($mstrhak0r_create_write_file) 
		{
		  fwrite($mstrhak0r_create_write_file, $mstrhak0r_file_ip);
...
elseif(!file_exists($mstrhak0r_server_path."/".name_of_file_to_save_user_details))
{	
...
function mstrhak0r_format_fullnames($name=NULL) 
{
	if (empty($name))
		return false;
...
if(isset($_POST['page']) && !empty($_POST['page']))
{
	...
	{
		if(isset($_POST['mstrhak0r_fullname']) && isset($_POST['mstrhak0r_username']) && isset($_POST['mstrhak0r_email']) && isset($_POST['mstrhak0r_passwd']) && !empty($_POST['mstrhak0r_fullname']) && !empty($_POST['mstrhak0r_username']) && !empty($_POST['mstrhak0r_email']) && !empty($_POST['mstrhak0r_passwd']))
		{
			$fullname = mstrhak0r_format_fullnames(trim(strip_tags($_POST['mstrhak0r_fullname'])));
			$username = trim(strip_tags($_POST['mstrhak0r_username']));
			$email = trim(strip_tags($_POST['mstrhak0r_email']));
			$passwd = trim(strip_tags($_POST['mstrhak0r_passwd']));
			...
			{
				$mstrhak0r_database = fopen(name_of_file_to_save_user_details,"a+");
				rewind($mstrhak0r_database);
			
				while (!feof($mstrhak0r_database)) 
				{
					$mstrhak0r_get_db_lines = fgets($mstrhak0r_database);
					$mstrhak0r_fetch_detail = explode('::::::::::', $mstrhak0r_get_db_lines);
					
					/*User Information are shown below:*/
					//$mstrhak0r_fetch_detail[0] = Fullname
					//$mstrhak0r_fetch_detail[1] = Username
					//$mstrhak0r_fetch_detail[2] = Email Address
					//$mstrhak0r_fetch_detail[3] = Password
					
					$mstrhak0r_account_username = base64_decode(trim(strip_tags($mstrhak0r_fetch_detail[1])));
					$mstrhak0r_account_email = base64_decode(trim(strip_tags($mstrhak0r_fetch_detail[2])));
					
					// Username validation to avoid duplicates
					if ($mstrhak0r_account_username == $username) 
					{
						$mstrhak0r_error = '<div class="info">Sorry, the username <b>'.$username.'</b> has already been taken by someone else. <br>Please enter a different username to proceed. Thanks.</div>';
						break;
					}
					// Email Address validation to avoid duplicates
					elseif ($mstrhak0r_account_email == $email) 
					{
						$mstrhak0r_error = '<div class="info">Sorry, <b>'.$email.'</b> already exist on this system and duplicate email addresses are not allowed for security reasons. <br>Please enter a different email address to proceed. Thanks.</div>';
						break;
					}
				}
				...
				{
					echo $mstrhak0r_error;
				}
				fclose($mstrhak0r_database);
			}
		}
		else
		{
	...
	{
		if(isset($_POST['mstrhak0r_username']) && isset($_POST['mstrhak0r_passwd']) && !empty($_POST['mstrhak0r_username']) && !empty($_POST['mstrhak0r_passwd']))
		{
			$username = trim(strip_tags($_POST['mstrhak0r_username']));
			$passwd = trim(strip_tags($_POST['mstrhak0r_passwd']));
			$encrypted_user_password = md5($passwd);
			...
			{
				$mstrhak0r_databases = fopen(name_of_file_to_save_user_details,"r");
				rewind($mstrhak0r_databases);
			
				while (!feof($mstrhak0r_databases)) 
				{
					$mstrhak0r_get_db_line = fgets($mstrhak0r_databases);
					$mstrhak0r_fetch_details = explode('::::::::::', $mstrhak0r_get_db_line);
					...
					$mstrhak0r_account_username = base64_decode(trim($mstrhak0r_fetch_details[1]));
					$mstrhak0r_account_password = base64_decode(trim($mstrhak0r_fetch_details[3]));
					
					// Validate Username
					if (!empty($mstrhak0r_account_username) && !empty($mstrhak0r_account_password) && $mstrhak0r_account_username == $username) 
					{
						// Username is valid therefore, validate user password
						if ($mstrhak0r_account_password == $encrypted_user_password)
						{
							...
							$_SESSION['validfullname'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[0])));
							$_SESSION['validusername'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[1])));
							$_SESSION['validemail'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[2])));
							$_SESSION['validpassword'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[3])));
						}
						else
						{
							$mstrhak0r_error = '<div class="info">Sorry, the information you have provided are incorrect. Please enter your valid information to access this system. Thanks.</div>';
							break;
						}
					}
				}
	...
	{
		if(isset($_POST['mstrhak0r_username']) && !empty($_POST['mstrhak0r_username']))
		{
			$username = trim(strip_tags($_POST['mstrhak0r_username']));
			...
			else
			{
				...
				while (!feof($mstrhak0r_databases)) 
				{
					...
					$mstrhak0r_account_username = base64_decode(strip_tags($mstrhak0r_fetch_details[1]));
					
					// Validate Username
					if(!empty($mstrhak0r_account_username))
					{
						if($mstrhak0r_account_username == $username) 
						{
							...
							//Don't forget that the user information were encrypted before they were saved to a file hence the decoding below
							$_SESSION['mstrhak0r_fullname'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[0])));
							$_SESSION['mstrhak0r_username'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[1])));
							$_SESSION['mstrhak0r_email'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[2])));
						}
					}
				}
				...
				if ($mstrhak0r_error == '')
				{
					if(isset($_SESSION['mstrhak0r_fullname']) && isset($_SESSION['mstrhak0r_username']) && isset($_SESSION['mstrhak0r_email']))
					{
						$the_date = date("F jS Y");
						$the_time = date('g:i A');
						$ForgotPasswordYear = date("Y");
						$fullname = $_SESSION['mstrhak0r_fullname'];
						$username = $_SESSION['mstrhak0r_username'];
						$email = $_SESSION['mstrhak0r_email'];
						
						$encrypted_fullname = base64_encode($_SESSION['mstrhak0r_fullname']);
						$encrypted_username = base64_encode($_SESSION['mstrhak0r_username']);
						$encrypted_email = base64_encode($_SESSION['mstrhak0r_email']);
   ...
   else
   {
	...
	elseif($_POST['page'] == "reset_password")
	{
		if(isset($_POST['hidden_username']) && isset($_POST['new_password']) && !empty($_POST['hidden_username']) && !empty($_POST['new_password']))
		{
			...
			{
				$mstrhak0r_databases = fopen(name_of_file_to_save_user_details,"r"); //"a+"
				rewind($mstrhak0r_databases);
				...
				while (!feof($mstrhak0r_databases)) 
				{
					...
					if ($mstrhak0r_account_username == $username) 
					{
						$mstrhak0r_old_account_password = trim(strip_tags($mstrhak0r_fetch_details[3]));
						
						$mstrhak0r_databases_file_name = name_of_file_to_save_user_details;
						$mstrhak0r_file_handler = fopen($mstrhak0r_databases_file_name,"r");
						$mstrhak0r_content_reader = fread($mstrhak0r_file_handler,filesize($mstrhak0r_databases_file_name));
						
						$mstrhak0r_content_reader = str_replace($mstrhak0r_old_account_password, base64_encode($encrypted_user_new_password), $mstrhak0r_content_reader);
						
						$mstrhak0r_file_handler = fopen($mstrhak0r_databases_file_name,"w");
						
						if(fwrite($mstrhak0r_file_handler,$mstrhak0r_content_reader))
						{
							$_SESSION['mstrhak0r_reset_fullname'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[0])));
							$_SESSION['mstrhak0r_reset_username'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[1])));
							$_SESSION['mstrhak0r_reset_email'] = base64_decode(trim(strip_tags($mstrhak0r_fetch_details[2])));
							fclose($mstrhak0r_file_handler);
						}
						else
						{
							$mstrhak0r_error = '<div class="info">Sorry, your account password could not be changed at this time. Please try again later or contact this website admin to report the error message if this problem persist (1). Thanks.</div>';
							break;
						}
					}
				}
				...
				if ($mstrhak0r_error == '')
				{
					if(isset($_SESSION['mstrhak0r_reset_fullname']) && isset($_SESSION['mstrhak0r_reset_username']) && isset($_SESSION['mstrhak0r_reset_email']))
					{
						echo '<font style="font-size:0px;">completed</font>';
						echo '<div class="info" align="left">Congrats <b>'.$_SESSION['mstrhak0r_reset_fullname'].'</b>,<br><br>Your account password has been changed successfully. <br><br>If you did not see the Back to Login button to click on it so as to login with your new password, please refresh this page to re-login with your new account password.<br><br>Thank You!</div>';
						session_unset();
						session_destroy();
					}
					else
